...
class MyPagenation():
    ...
    def __init__(self, page_num, total_count,base_url,get_data=None,per_page_num=10, page_num_show=5):
        ...
        self.per_page_num = per_page_num  # 搜索条件保留
        self.get_data = get_data  # 每页显示10条
        # 页面生成页码的数量
    ...
    def page_hmtl(self):
        ...
        first_page_html = '<li><a href="{1}?{0}" aria-label="Previous"><span aria-hidden="true">首页</span></a></li>'.format(
            self.get_data.urlencode(), self.base_url)
        page_html += first_page_html
        if self.page_num <= 1:
            pre_page = '<li class="disabled"><a href="javascript:void(0)" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>'.format(
                self.page_num - 1)
        else:
            self.get_data['page'] = self.page_num - 1
            pre_page = '<li><a href="{1}?{0}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>'.format(
                self.get_data.urlencode(), self.base_url)
        ...
        for i in page_num_range:
            ...
            if i == self.page_num:
                ...
                page_html += '<li><a href="{1}?{2}">{0}</a></li>'.format(i, self.base_url,
